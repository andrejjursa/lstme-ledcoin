    ...
    protected function get_persons_graph_drilldown_json($person_id, $filter_orderby) {
        ...
        foreach ($operations as $operation) {
            ...
            if ($operation->type == Operation::TYPE_ADDITION && (int)$operation->amount > 0) {
                $days_plus_amount[$day_index] += (int)$operation->amount;
            } elseif ($operation->type == Operation::TYPE_SUBTRACTION) {
                if ($operation->subtraction_type == Operation::SUBTRACTION_TYPE_DIRECT && (int)$operation->amount > 0) {
                    $days_minus_amount[$day_index] += (int)$operation->amount;
                    ...
                    $days_minus_amount[$day_index] += (int)$operation->product_quantity_quantity * (int)$operation->product_quantity_price;
                    ...
                    $days_minus_amount[$day_index] += (int)$operation->service_usage_quantity * (int)$operation->service_usage_price;
                }
            }
        }
