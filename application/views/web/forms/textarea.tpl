<div class="field_wrap" {include file="web/partials/data_attributes.tpl" data=$form_element.data inline}>
    <div class="ui-field-contain{if $form_element.class} {$form_element.class|escape:'html'}{/if}">
        ...
        <textarea name="{$form_element.name|default:'unknown_name'}" {if $form_element.id}id="{$form_element.id}"{/if}
                {if $form_element.placeholder} placeholder="{$form_element.placeholder|escape:'html'}"{/if}
                ...
                {if $form_element.monospace eq TRUE}style="font-family: monospace"{/if}
        >{form_value|escape:'html' default=$form_element.default|default:'' source=$form_source|default:'' name=$form_element.name|default:'' property=$form_element.object_property|default:''}</textarea>
    </div>
