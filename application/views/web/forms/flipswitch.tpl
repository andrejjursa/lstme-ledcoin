<div class="field_wrap" {include file="web/partials/data_attributes.tpl" data=$form_element.data inline}>
    <div class="ui-field-contain{if $form_element.class} {$form_element.class|escape:'html'}{/if}">
        ...
        <select name="{$form_element.name|default:'unknown_name'}" {if $form_element.id}id="{$form_element.id}"{/if} data-role="slider" {if form_error($form_element.name) neq ''}data-theme="b"{/if} {if $form_element.disabled eq TRUE}disabled="disabled"{/if}>
